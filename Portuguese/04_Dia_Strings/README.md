...
[<< Dia 3](../03_Dia_Operadores/README.md) | [Dia 5 >>](../05_Dia_Listas/README.md)
...
Texto é um tipo de dado string. Qualquer dado escrito como texto é uma string. Qualquer dado entre aspas simples, duplas ou triplas é string. Existem diferentes métodos de string e funções nativas para lidar com tipos de dados string. Para verificar o comprimento de uma string, use o método len().
...
print(letter)               # P
print(len(letter))          # 1
greeting = 'Hello, World!'  # Uma string pode ser feita com aspas simples ou duplas, "Hello, World!"
...
Uma string multilinha é criada usando aspas simples triplas (''') ou aspas duplas triplas ("""). Veja o exemplo abaixo.
...
Podemos conectar strings. Mesclar ou conectar strings se chama concatenação. Veja o exemplo abaixo:
...
# Verificando o comprimento de uma string com a função nativa len()
...
print('Days\tTopics\tExercises') # adicionando espaço de tab ou 4 espaços
print('Day 1\t5\t5')
print('Day 2\t6\t20')
print('Day 3\t5\t23')
print('Day 4\t1\t35')
...
Em Python existem muitas formas de formatar strings. Nesta seção, vamos cobrir algumas delas.
O operador "%" é usado para formatar um conjunto de variáveis envolvidas em uma "tupla" (uma lista de tamanho fixo), junto com uma string de formato, que contém texto normal junto com "especificadores de argumento", símbolos especiais como "%s", "%d", "%f", "%.<small>número de dígitos</small>f".
...
Esse formato foi introduzido na versão 3 do Python.
...
Outra formatação nova de strings é a interpolação, as f-strings. As strings começam com f e podemos injetar os dados nas posições correspondentes.
...
Strings em Python são sequências de caracteres e compartilham seus métodos básicos de acesso com outras sequências ordenadas de objetos do Python — listas e tuplas. A forma mais simples de extrair caracteres individuais de strings (e membros individuais de qualquer sequência) é desempacotá-los em variáveis correspondentes.
...
Na programação, a contagem começa do zero. Portanto, a primeira letra de uma string está no índice zero e a última letra de uma string é o comprimento da string menos um.

![Índice da string](../../images/string_index.png)
...
Se quisermos começar da extremidade direita, podemos usar indexação negativa. -1 é o último índice.
...
- count(): retorna as ocorrências de uma substring na string, count(substring, start=.., end=..). O start é o índice inicial para a contagem e o end é o último índice a contar.
...
- expandtabs(): Substitui o caractere de tab por espaços; o tamanho padrão da tab é 8. Recebe o argumento do tamanho da tab
...
- rfind(): Retorna o índice da última ocorrência de uma substring; se não encontrar, retorna -1
...
- format(): formata a string em uma saída mais agradável  
   Saiba mais sobre formatação de strings neste [link](https://www.programiz.com/python-programming/methods/string/format)
...
- index(): Retorna o menor índice de uma substring; argumentos adicionais indicam o índice inicial e final (padrão 0 e comprimento da string - 1). Se a substring não for encontrada, lança um ValueError.
...
- rindex(): Retorna o maior índice de uma substring; argumentos adicionais indicam o índice inicial e final (padrão 0 e comprimento da string - 1)
...
- isalnum(): Verifica caractere alfanumérico
...
```py
challenge = 'thirty days of python'
print(challenge.isalpha()) # False, o espaço mais uma vez é excluído
challenge = 'ThirtyDaysPython'
...
challenge = '12 3'
print(challenge.isdecimal())  # False, espaço não é permitido
```
...
- isnumeric(): Verifica se todos os caracteres em uma string são números ou relacionados a números (assim como isdigit(), só que aceita mais símbolos, como ½)
...
- isidentifier(): Verifica se é um identificador válido — verifica se uma string é um nome de variável válido
...
- islower(): Verifica se todos os caracteres alfabéticos da string estão em minúsculas
...
- isupper(): Verifica se todos os caracteres alfabéticos da string estão em maiúsculas
...
- replace(): Substitui uma substring por uma string fornecida
...
- split(): Divide a string, usando a string fornecida ou o espaço como separador
...
🌕 Você é uma pessoa extraordinária e tem um potencial notável. Você acabou de concluir os desafios do dia 4 e está quatro passos à frente no caminho para a grandeza. Agora faça alguns exercícios para o cérebro e para os músculos.
...
8. Use os métodos capitalize(), title() e swapcase() para formatar o valor da string _Coding For All_.
9. Recorte (slice) a primeira palavra da string _Coding For All_.
...
13. Divida a string 'Coding For All' usando o espaço como separador (split()).
14. "Facebook, Google, Microsoft, Apple, IBM, Oracle, Amazon" — divida a string na vírgula.
15. Qual é o caractere no índice 0 na string _Coding For All_.
16. Qual é o último índice da string _Coding For All_.
17. Qual caractere está no índice 10 na string "Coding For All".
18. Crie um acrônimo ou uma abreviação para o nome 'Python For Everyone'.
19. Crie um acrônimo ou uma abreviação para o nome 'Coding For All'.
...
25. Recorte a frase 'because because because' na seguinte sentença: 'You cannot end a sentence with because because because is a conjunction'
...
27. Recorte a frase 'because because because' na seguinte sentença: 'You cannot end a sentence with because because because is a conjunction'
...
33. Use a sequência de escape de nova linha para separar as seguintes frases.
    ...
    ```
34. Use uma sequência de escape de tab para escrever as seguintes linhas.
    ```py
