...
[<< Dia 6](../06_Dia_Tuplas/README.md) | [Dia 8 >>](../08_Dia_Dicionarios/README.md)
...
- [📘 Dia 7](#-dia-7)
  - [Conjuntos](#conjuntos)
    ...
    - [Obtendo o comprimento do conjunto](#obtendo-o-comprimento-do-conjunto)
    ...
    - [Encontrando itens da interseção](#encontrando-itens-da-interseção)
...
Conjunto (set) é uma coleção de itens. Deixe-me levá-lo de volta à sua aula de Matemática do ensino fundamental ou médio. A definição matemática de conjunto também pode ser aplicada em Python. Conjunto é uma coleção de elementos distintos, não ordenados e sem índice. Em Python, set é usado para armazenar itens únicos, e é possível encontrar a _união_, _interseção_, _diferença_, _diferença simétrica_, _subconjunto_, _superconjunto_ e _conjuntos disjuntos_ entre conjuntos.

### Criando um conjunto

Para criar um conjunto vazio, usamos a função set(). Chaves vazias {} criam um dicionário.
...
Usamos loops para acessar os itens. Veremos isso na seção de loops

### Verificando um item

Para verificar se um item existe em um conjunto, usamos o operador de pertinência _in_.

```py
# sintaxe
st = {'item1', 'item2', 'item3', 'item4'}
print("O conjunto st contém item3? ", 'item3' in st) # O conjunto st contém item3? True
```
...
Depois que um conjunto é criado, não podemos alterar nenhum item, mas também podemos adicionar itens adicionais.
...
- Adicione vários itens usando _update()_
  O _update()_ permite adicionar vários itens a um conjunto. O _update()_ recebe um argumento de lista.
...
Podemos remover um item de um conjunto usando o método _remove()_. Se o item não for encontrado, o método _remove()_ vai gerar erros, então é bom verificar se o item existe no conjunto dado. Porém, o método _discard()_ não gera nenhum erro.
...
Os métodos pop() removem um item aleatório de um conjunto e retornam o item removido.
...
Podemos converter lista em conjunto e conjunto em lista. Converter lista em conjunto remove duplicatas e apenas itens únicos serão preservados.

```py
# sintaxe
lst = ['item1', 'item2', 'item3', 'item4', 'item1']
st = set(lst)  # {'item2', 'item4', 'item1', 'item3'} - a ordem é aleatória, porque conjuntos em geral não são ordenados
```
...
- Update
  Este método insere um conjunto em um conjunto dado
...
### Encontrando itens da interseção
...
Um conjunto pode ser um subconjunto ou um superconjunto de outros conjuntos:
...
Retorna a diferença simétrica entre dois conjuntos. Isso significa que retorna um conjunto que contém todos os itens de ambos os conjuntos, exceto os itens que estão presentes em ambos, matematicamente: (A\B) ∪ (B\A)
...
Se dois conjuntos não têm um item ou itens em comum, chamamos de conjuntos disjuntos. Podemos verificar se dois conjuntos têm itens em comum ou são disjuntos usando o método _isdisjoint()_.
...
python = {'p', 'y', 't', 'h', 'o','n'}
dragon = {'d', 'r', 'a', 'g', 'o','n'}
python.isdisjoint(dragon)  # False, há itens em comum {'o', 'n'}
```

🌕 Você é uma estrela em ascensão. Você acabou de concluir os desafios do dia 7 e está 7 passos à frente no caminho para a grandeza. Agora faça alguns exercícios para o cérebro e para os músculos.
...
1. Converta as idades em um conjunto e compare o comprimento da lista e do conjunto; qual é maior?
2. Explique a diferença entre os seguintes tipos de dados: string, list, tuple e set
